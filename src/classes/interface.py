...
class canvas:
    ...
    def ratio(self, window_dimensions):
        return vec2(self.display_dimensions.x / window_dimensions.x, self.display_dimensions.y / window_dimensions.y)
    ...
    def relative(self, object_position: vec2, window_dimensions: vec2):
        ...
        canvas_dimension_ratio = vec2(self.internal_dimensions.x / window_dimensions.x, self.internal_dimensions.y / window_dimensions.y)

        # Squash the object's original position down to match the ratio. (Note this isn't aligned correctly yet, hence the next step)
        canvas_dimension_position = vec2(canvas_dimension_ratio.x * object_position.x, canvas_dimension_ratio.y * object_position.y)
        ...
        canvas_display_ratio = vec2(canvas_dimension_position.x / self.internal_dimensions.x, canvas_dimension_position.y / self.internal_dimensions.y)
        ...
        canvas_display_position = vec2(self.display_dimensions.x * canvas_display_ratio.x, self.display_dimensions.y * canvas_display_ratio.y)

        # An offset of the object to the canvas's position. This is the true position of the object.
        final_position = vec2(self.position.x + canvas_display_position.x, self.position.y + canvas_display_position.y)
